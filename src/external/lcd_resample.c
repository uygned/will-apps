  ...
  case -1:
	for(y=0; y<h; y++)
	  for(x=0; x<w; x++) {
		...
		b =
		  c[0].b*c0 + c[5].b*c2 + c[10].b*c0 +
		  c[1].b*c1 + c[6].b*c3 + c[11].b*c1 +
		  c[2].b*c0 + c[7].b*c2 + c[12].b*c0;

		// Calculate green component from centre pixel block
		g =
		  c[1].g*c0 + c[6].g*c2 + c[11].g*c0 +
		  c[2].g*c1 + c[7].g*c3 + c[12].g*c1 +
		  c[3].g*c0 + c[8].g*c2 + c[13].g*c0;

		// Calculate red component from rightmost pixel block
		r =
		  c[2].r*c0 + c[7].r*c2 + c[12].r*c0 +
		  c[3].r*c1 + c[8].r*c3 + c[13].r*c1 +
		  c[4].r*c0 + c[9].r*c2 + c[14].r*c0;

		// Calculate alpha from the centre pixel block. XXX: This is
		// probably not the correct value, but it looks okay.
		a =
		  c[1].a*c0 + c[6].a*c2 + c[11].a*c0 +
		  c[2].a*c1 + c[7].a*c3 + c[12].a*c1 +
		  c[3].a*c0 + c[8].a*c2 + c[13].a*c0;
  ...
  case 1:
	for(y=0; y<h; y++)
	  for(x=0; x<w; x++) {
		...
		r =
		  c[0].r*c0 + c[5].r*c2 + c[10].r*c0 +
		  c[1].r*c1 + c[6].r*c3 + c[11].r*c1 +
		  c[2].r*c0 + c[7].r*c2 + c[12].r*c0;
		...
		b =
		  c[2].b*c0 + c[7].b*c2 + c[12].b*c0 +
		  c[3].b*c1 + c[8].b*c3 + c[13].b*c1 +
		  c[4].b*c0 + c[9].b*c2 + c[14].b*c0;
  ...
  case -2:
	for(y=0; y<h; y++)
	  for(x=0; x<w; x++) {
		...
		// Calculate red component from topmost pixel block
		r =
		  c[1].r*c0 + c[6].r*c2 + c[11].r*c0 +
		  c[2].r*c1 + c[7].r*c3 + c[12].r*c1 +
		  c[3].r*c0 + c[8].r*c2 + c[13].r*c0;

		// Calculate green component from centre pixel block
		g =
		  c[6].g*c0 + c[11].g*c2 + c[16].g*c0 +
		  c[7].g*c1 + c[12].g*c3 + c[17].g*c1 +
		  c[8].g*c0 + c[13].g*c2 + c[18].g*c0;

		// Calculate blue component from bottommost pixel block
		b =
		  c[11].b*c0 + c[16].b*c2 + c[21].b*c0 +
		  c[12].b*c1 + c[17].b*c3 + c[22].b*c1 +
		  c[13].b*c0 + c[18].b*c2 + c[23].b*c0;

		// Calculate alpha from the centre pixel block. XXX: This is
		// probably not the correct value, but it looks okay.
		a =
		  c[6].a*c0 + c[11].a*c2 + c[16].a*c0 +
		  c[7].a*c1 + c[12].a*c3 + c[17].a*c1 +
		  c[8].a*c0 + c[13].a*c2 + c[18].a*c0;
  ...
  case 2:
	for(y=0; y<h; y++)
	  for(x=0; x<w; x++) {
		...
		// Calculate blue component from topmost pixel block
		b =
		  c[1].b*c0 + c[6].b*c2 + c[11].b*c0 +
		  c[2].b*c1 + c[7].b*c3 + c[12].b*c1 +
		  c[3].b*c0 + c[8].b*c2 + c[13].b*c0;
		...
		// Calculate red component from bottommost pixel block
		r =
		  c[11].r*c0 + c[16].r*c2 + c[21].r*c0 +
		  c[12].r*c1 + c[17].r*c3 + c[22].r*c1 +
		  c[13].r*c0 + c[18].r*c2 + c[23].r*c0;
  ...
  default:
	for(y=0; y<h; y++)
	  for(x=0; x<w; x++) {
		...
		r =
		  c[6].r*c0 + c[11].r*c2 + c[16].r*c0 +
		  c[7].r*c1 + c[12].r*c3 + c[17].r*c1 +
		  c[8].r*c0 + c[13].r*c2 + c[18].r*c0;
		...
		b =
		  c[6].b*c0 + c[11].b*c2 + c[16].b*c0 +
		  c[7].b*c1 + c[12].b*c3 + c[17].b*c1 +
		  c[8].b*c0 + c[13].b*c2 + c[18].b*c0;
...
int usage(char *exe)
{
  printf("Usage:\n\t%s -i <fn> -o <fn> [-x <0-2>] [-y <0-2>] [-s <0-3>] [-v] [-r]\n", exe);
  return 1;
}
...
int main(int argc, char **argv)
{
  ...
  char *out_fn = NULL;
  char *in_fn = NULL;
  int strength = -1, xoff = 0, yoff = 0, order = 1;
  int ch, optlen;

  while ((ch = getopt(argc, argv, "rvno:i:s:x:y:")) != -1) {
	switch (ch) {
	case 'o':
	  out_fn = (char *)calloc(optlen = strlen(optarg)+1,1);
	  out_fn = strcpy(out_fn, optarg);
	  break;

	case 'i':
	  in_fn = (char *)calloc(optlen = strlen(optarg)+1,1);
  ...
  lcd_resample(spix, dpix, w3, h3, w, h, xoff, yoff, strength, order);
